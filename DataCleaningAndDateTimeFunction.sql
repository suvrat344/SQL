...
UPDATE flights 
SET 
    departure = STR_TO_DATE(CONCAT(Date_of_Journey, ' ', Dep_Time),'%Y-%m-%d %H:%i');
    
ALTER TABLE flights ADD COLUMN arrival DATETIME;
UPDATE flights 
SET 
    arrival = DATE_ADD(departure,INTERVAL duration MINUTE);
SELECT 
    TIME(arrival)
FROM
    flights;
...
SELECT 
    source,
    destination,
    TIME_FORMAT(SEC_TO_TIME(AVG(duration) * 60),'%kh:%im') AS AverageDuration
FROM
    flights
GROUP BY 
	source,
...
SELECT 
	*,
	CASE WHEN total_stops = "non-stop" THEN "non-stop" ELSE "with stop" END AS temp
FROM 
flights
)
SELECT 
	temp,
	TIME_FORMAT(SEC_TO_TIME(AVG(duration)*60),"%kh : %im") AS AvearageDuration 
FROM 
	temp_table 
GROUP BY 
	temp;
...
SELECT 
    airline,
    TIME_FORMAT(SEC_TO_TIME(MAX(Duration) * 60),'%kh : %im') AS MaxDuration
FROM
    flights
GROUP BY 
	Airline

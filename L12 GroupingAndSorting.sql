...
SELECT age,LightSleepPercentage,DeepSleepPercentage FROM sleep_efficiency WHERE DeepSleepPercentage BETWEEN 25 AND 45 ORDER BY LightSleepPercentage 
LIMIT 10,20;
...
SELECT ExerciseFrequency,SmokingStatus,AVG(DeepSleepPercentage * SleepDuration / 100) as DeepSleep,AVG(LightSleepPercentage * SleepDuration / 100),
AVG(RemSleepPercentage * SleepDuration / 100) FROM sleep_efficiency GROUP BY ExerciseFrequency,SmokingStatus ORDER BY DeepSleep;
...
SELECT "Power Station",AVG("Monitored Cap.(MW)") AS "AVG MonitoredCap",COUNT(*) AS occurence FROM PowerGeneration GROUP BY "Power Station" 
HAVING "AVG MonitoredCap" BETWEEN 1000 AND 2000 AND occurence > 200 ORDER BY "AVG MonitoredCap" ASC;


USE AverageCostOfUndergradCollegeByState;
SELECT * FROM nces;
...
SELECT state,ROUND(AVG(value),2) AS AvgValue,COUNT(*) AS frequency FROM nces WHERE type = "Public In-State" AND year in (2013,2017,2021) GROUP BY 
state HAVING frequency BETWEEN 6 AND 10 ORDER BY AvgValue ASC LIMIT 10;
...
SELECT state,AVG(value) as AverageValue FROM nces WHERE type LIKE  "Private" AND year = 2021  GROUP BY state ORDER BY AverageValue DESC LIMIT 1,1;

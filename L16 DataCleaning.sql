...
ScreenResolution IS NULL AND Cpu IS NULL AND Ram IS NULL AND Memory  IS NULL AND Gpu IS NULL AND OpSys IS NULL AND Weight IS NULL AND Price IS NULL)
AS t);

-- Drop Duplicates
SELECT Company,TypeName,Inches,ScreenResolution,Cpu,Ram,Memory,Gpu,OpSys,Weight,Price,COUNT(*) FROM laptop GROUP BY Company,TypeName,Inches,
ScreenResolution,Cpu,Ram,Memory,Gpu,OpSys,Weight,Price HAVING COUNT(*) > 1;

DELETE FROM laptop WHERE id NOT IN (SELECT * FROM (SELECT MIN(id) FROM laptop GROUP BY Company,TypeName,Inches,ScreenResolution,Cpu,Ram,Memory,Gpu,
OpSys,Weight,Price) AS t);
...
ALTER TABLE laptop ADD COLUMN cpu_brand VARCHAR(255) AFTER Cpu,ADD COLUMN cpu_name VARCHAR(255) AFTER cpu_brand,ADD COLUMN cpu_speed DECIMAL(10,1)
AFTER cpu_name;
...
UPDATE laptop SET resolution_width = SUBSTRING_INDEX(SUBSTRING_INDEX(ScreenResolution,' ',-1),'x',1),resolution_height = SUBSTRING_INDEX(SUBSTRING_INDEX(ScreenResolution,' ',-1),
'x',-1);
...
ALTER TABLE laptop ADD COLUMN memory_type VARCHAR(255) AFTER memory,ADD COLUMN primary_storage INT AFTER memory_type,ADD COLUMN secondary_storage INT AFTER primary_storage;
...
CASE
    ...
    ELSE NULL
END;
UPDATE laptop SET primary_storage = REGEXP_SUBSTR(SUBSTRING_INDEX(memory,"+",1),'[0-9]+'),secondary_storage = CASE WHEN memory LIKE '%+%' THEN REGEXP_SUBSTR(SUBSTRING_INDEX(memory,
"+",-1),'[0-9]') ELSE 0 END;
UPDATE laptop SET primary_storage = CASE WHEN primary_storage <= 2 THEN primary_storage * 1024 ELSE primary_storage END,secondary_storage = CASE WHEN secondary_storage <= 2 
...
SELECT source,destination,TIME_FORMAT(SEC_TO_TIME(AVG(duration)*60),'%kh:%im') AS AverageDuration FROM flights GROUP BY source,destination;
...
SELECT airline,TIME_FORMAT(SEC_TO_TIME(MAX(Duration)*60),"%kh : %im") AS MaxDuration FROM flights GROUP BY Airline ORDER BY MaxDuration DESC;

-- Find all the pair of cities having average time duration > 3 hours.
SELECT source,destination,TIME_FORMAT(SEC_TO_TIME(AVG(Duration)*60),"%kh : %im") AS AverageDuration FROM flights GROUP BY source,destination HAVING AVG(duration) > 180; 

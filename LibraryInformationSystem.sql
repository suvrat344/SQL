...
-- Question1 : Write an SQL statement to find the titles of books authored by an author having first name as 'Joh Paul' and last name as '
-- Mueller'.
SELECT book_catalogue.title FROM book_authors,book_catalogue WHERE book_authors.ISBN_no=book_catalogue.ISBN_no AND book_authors.author_fname
='Joh Paul' AND book_authors.author_lname='Mueller';

-- Question2 : Write a SQL statement to find the titles of books published by 'McGraw Hill Education'.
SELECT title FROM book_catalogue WHERE publisher= 'McGraw Hill Education';

-- Question3 : Write a SQL statement to display the first name and the last name of students (student_fname, student_lname) pursuing 'PG' 
-- courses.
SELECT student_fname,student_lname FROM students WHERE degree = 'MCA';

-- Question3 : Write an SQL statement to find the first names and mobile number (student_fname, mobile_no) of students who belong to the 
-- department with department code as 'EE' and who were born after '2001-06-15'.
SELECT student_fname,mobile_no FROM students WHERE  department_code='EE' AND dob>'2001-06-15';

-- Question4 : Write an SQL statement to find the last names (faculty_lname) of female faculty who belong to the department:'Mechanical 
-- Engineering'.
SELECT f.faculty_lname FROM faculty f,departments d WHERE f.department_code=d.department_code AND d.department_name='Mechanical Engineering'
AND f.gender='F';

-- Question5 : Write an SQL statement to find the first name,last name of the faculty of the department having department code as "ME" and 
-- who have issued atleast one book,such that there are no duplicates firstname,lastname pairs.
SELECT faculty_fname,faculty_lname FROM faculty INNER JOIN members ON faculty.id=members.roll_no WHERE department_code="ME" AND 
member_no IN (SELECT member_no FROM book_issue NATURAL JOIN members);
 
-- Question6 : Write an SQL statement to find the number of book-titles issued on 11th August 2021.
SELECT count(*) FROM book_issue GROUP BY doi HAVING doi = '2021-08-11';

-- Question7 : Write a SQL statement to find the names of faculty (faculty_fname, faculty_lname) who did not issue any book.
SELECT faculty_fname,faculty_lname FROM faculty WHERE faculty.id NOT IN (SELECT members.id FROM members NATURAL JOIN book_issue 
WHERE members.id IS NOT NULL);

-- Question8 : Write a SQL statement to find the unique book titles which are issued to 'PG' students but not to 'UG' students .
SELECT DISTINCT(title) FROM book_catalogue NATURAL JOIN book_copies WHERE accession_no IN (SELECT accession_no FROM book_issue WHERE
member_no IN (SELECT member_no FROM members WHERE member_type="PG")) EXCEPT(SELECT DISTINCT(title) FROM book_catalogue NATURAL JOIN 
book_copies WHERE accession_no IN (SELECT accession_no FROM book_issue WHERE member_no IN (SELECT member_no FROM members WHERE member_type=
"UG")));

SELECT title FROM book_catalogue WHERE isbn_no IN (SELECT isbn_no FROM book_issue NATURAL JOIN book_copies WHERE member_no IN (SELECT 
member_no FROM members NATURAL JOIN book_issue WHERE member_type = 'PG') EXCEPT SELECT isbn_no FROM book_issue NATURAL JOIN book_copies
WHERE member_no IN (SELECT member_no FROM members NATURAL JOIN book_issue WHERE member_type = 'UG'));

--  Question9 : Write an SQL statement to find student_fname and member_no of all students who have issued (borrowed) books on '2021-08-01'. 
SELECT students.student_fname,members.member_no FROM students INNER JOIN members ON students.roll_no = members.roll_no WHERE 
members.member_no IN (SELECT member_no FROM book_issue WHERE doi='2021-08-01');

-- Question10 : Write an SQL statement to find the book titles and the number of copies of the books which has the word 'Easy' in their 
-- title. 
SELECT book_catalogue.title,count(book_catalogue.title) FROM book_catalogue INNER JOIN book_copies ON book_catalogue.ISBN_no = 
book_copies.ISBN_no WHERE book_catalogue.title LIKE '%Easy%' GROUP BY book_catalogue.title;

-- Question11 : Write an SQL query to find the roll number (roll_no) and the member number (member_no) of each student who belongs to either
-- the 'ME' or 'CS' department code.
SELECT students.roll_no, members.member_no FROM students
INNER JOIN
members ON students.roll_no = members.roll_no WHERE students.department_code IN ('ME' , 'CS');

-- Question12 : Find the first names and last names of authors, having the author’s first name as a single character.
SELECT author_fname, author_lname FROM book_authors WHERE author_fname LIKE '_';

-- Question13 : Find the titles and publishers of all books, except the ones published in year ‘2015’ or‘2017’.
SELECT title,publisher FROM book_catalogue  EXCEPT SELECT title,publisher FROM book_catalogue WHERE year IN (2015,2017);

-- Question14 : Find the first names and last names of the students whose birthday is in May 2002 or in May 2003.
SELECT student_fname,student_lname FROM students WHERE dob BETWEEN '2003-05-01' AND '2003-05-31' UNION SELECT student_fname,student_lname 
FROM students WHERE dob BETWEEN '2002-05-01' AND '2002-05-31';

-- Question15 : Find out the total number of members in the UG with alias name or column header as ‘total member’.
SELECT count(member_no) as "total member" FROM members WHERE member_type="UG";

-- Question16 : Find out the number of female students in each department. Display department code and number of female students.
SELECT department_code,count(gender) as "Number of Female" FROM students WHERE gender = "F" GROUP BY department_code;

-- Question17 : Write a query to obtain the natural join between the tables, students and departments.
SELECT * FROM students NATURAL JOIN departments;

-- Question18 : Find the name of the department in which Gita Das is studying.
SELECT department_name FROM departments WHERE department_code = (SELECT department_code FROM students WHERE student_fname = "Gita" AND
student_lname = "DAS");

-- Question19 : Find the roll number of all male students, having their department building in 'Block_2'
SELECT roll_no FROM students WHERE gender = "M" AND department_code = (SELECT department_code FROM departments WHERE 
department_building="Block_2");

-- Question20 : Find the first name, last name and the roll number of students having their department building in ‘Block 1’.
SELECT student_fname,student_lname,roll_no FROM students WHERE department_code = (SELECT department_code FROM departments WHERE 
department_building = "Block_1");

-- Question21 : Find out the details of the members who have not issued any books.
SELECT * FROM members WHERE NOT EXISTS (SELECT * FROM book_issue WHERE members.member_no = book_issue.member_no);

-- Question22 : Write a SQL statement to find out the dates when one or more copies of the book having the title “Learning with Python” has 
-- been issued.
SELECT distinct(doi) FROM book_issue INNER JOIN book_copies USING(accession_no) INNER JOIN book_catalogue USING(ISBN_no) WHERE 
book_catalogue.title = "Learning with Python";

-- Question23 : Write a SQL statement to find out the names of research scholars (RS) who have not issued any books.
SELECT student_fname,student_lname FROM students WHERE roll_no IN (SELECT roll_no FROM members WHERE member_no IN (SELECT member_no FROM 
members WHERE member_type='RS' EXCEPT (SELECT distinct(member_no) FROM book_issue)));